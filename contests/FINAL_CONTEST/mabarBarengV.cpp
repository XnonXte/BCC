...
void solve() {
  ...
  //   if (sum > m) {
  //     ranges.push_back({abs(m - sum), {l + 1, r + 1}});
  //   }
  ...
  // FOREP(diff, range, ranges) {
  //   CO diff << ": " << range.F << " " << range.S << EN;
  // }

  // PII smallest = ranges[0].S;
  // CO smallest.F << " " << smallest.S << EN;
  ...
  if (binary_search(ALL(p), m)) {
    int ans = lower_bound(ALL(p), m) - p.begin();
    CO ans << " " << ans << EN;
  } else {
  }
}
...
/*
Metode sorting mungkin belum benar
*/