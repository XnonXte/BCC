...
void solve() {
  ...
  II(b);
  II(c);
  VI psums = {a, a + b, a + b + c};

  int step = 1;
  while (psums.back() < n) {
    switch (step) {
    case 1:
      psums.emplace_back(psums.back() + a);
      step++;
      break;
    case 2:
      psums.emplace_back(psums.back() + b);
      step++;
      break;
    case 3:
      psums.emplace_back(psums.back() + c);
      step = 1;
      break;
    }
  }

  CO(lower_bound(ALL(psums), n) - psums.begin()) + 1 << EN;
}
